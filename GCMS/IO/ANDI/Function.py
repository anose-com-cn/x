...
# from pycdf import *
ERR_CDF_READING=100000



def CDF_Reader(file_name):
    from netCDF4 import Dataset
#    file_name="C:\\Users\\qiaos\\Desktop\\raw\\centroid\\HA.cdf"
    file_name="C:/x/PyMS/data/gc01_0812_066.cdf"

#对于一个有scancount=5个scan，每个scan中分别有points=[100,101,102,99,105]个质谱点的图谱
#    vars=(
#    #以下数据分别只有5个，用来描述每个scan的信息
#    "scan_acquisition_time",
#    "a_d_sampling_rate",           #意义不大采样速率 
#    "a_d_coaddition_factor",       #
#    "scan_acquisition_time",       #扫描开始时间，也就是保留时间
#    "scan_duration",               #扫描持续时间，可以有，意义不大 
#    "inter_scan_time",             #可以有，但是意义不大
#    "resolution",                  #分辨率，意义不大，可以计算 
#    "actual_scan_number",          #意义不大；
#    "total_intensity",             #总离子流，很重要！！！！
#    "mass_range_min",              #没用
#    "mass_range_max",              #没用
#    "time_range_min",              #没用
#    "time_range_max",              #没用
#    "scan_index",                  #每个scan开始位置，[0,100,201，303,402]    
#    "point_count",                 #每个scan的点数，[100,101,102,99,105]
#    "flag_count",                  #扫描的极性？
#    #以下数据分别有sum(points)=507个，用来描述每个质谱信号的信息
#    "mass_values",                 #每个质谱信号的m/z
#    "time_values",                 #应该是时间信息，未记录
#    "intensity_values")            #每个质谱信号的强度信息 
    #读取cdf文件
    import os
    if not os.path.exists(file_name):
        print("'File doesn't exist")
    try:
        CDFfile = Dataset(file_name, 'r')
    except :
        print("Cannot open file '%s'" % file_name)        
    print (" -> Reading netCDF file '%s'" % (file_name)) 
    
    cdfdata = CDFfile.variables
    scan_list=[]
    scancount=len(cdfdata["scan_index"])
    scan_acquisition_time = []
    tenpercent=int(scancount/10)
    for scanindex in range(scancount):
        if scanindex % tenpercent == 0:
            print ("%d percent finished!!!" % (scanindex /tenpercent*10))
        if scanindex % 5 == 0:
            start_pos = cdfdata["scan_index"][scanindex]
            end_pos = cdfdata["scan_index"][scanindex]+cdfdata["point_count"][scanindex]
            scan_mass_list = cdfdata["mass_values"][start_pos:end_pos]
            scan_intensity_list=cdfdata["intensity_values"][start_pos:end_pos]
            scan_list.append(Scan(scan_mass_list, scan_intensity_list))
            scan_acquisition_time.append(float(cdfdata["scan_acquisition_time"][scanindex]))
    print("scan_acquisition_time",len(scan_acquisition_time));
    print("scan_list",len(scan_list));
    return(scan_list,scan_acquisition_time)
#QSL
...
# ne# tcdf_file_date_time_stamp   5 CHAR    20 20090114001531+1100
